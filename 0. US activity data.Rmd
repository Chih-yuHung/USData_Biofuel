...
US_Direct <- Fert_County_Crop %>%
  ...
  #Some crops in some states are usually sown in fall so their growing season is between Oct-April or Nov-May
  mutate(
    P = case_when(
      CROP == "CANOLA"  & STATE %in% c("OKLAHOMA", "KANSAS", "CALIFORNIA", "TEXAS") ~ NGROW_PRE,  
      CROP %in% c("CAMELINA", "BARLEY") & STATE == "CALIFORNIA"               ~ NGROW_PRE,
      CROP == "RYE"     & STATE == "NORTH DAKOTA"                             ~ NGROW_PRE,
      str_detect(CLASS, regex("^winter", ignore_case = TRUE))       ~ NGROW_PRE,
      CROP == "TRITICALE" & DOUBLE == "Y"                           ~ NGROW_PRE,
      TRUE                                                          ~ GROW_PRE
    ),
    PE = case_when(
      CROP == "CANOLA"  & STATE %in% c("OKLAHOMA", "KANSAS", "CALIFORNIA", "TEXAS") ~ NGROW_PET,  
      CROP %in% c("CAMELINA", "BARLEY") & STATE == "CALIFORNIA"               ~ NGROW_PET,
      CROP == "RYE"     & STATE == "NORTH DAKOTA"                             ~ NGROW_PET,
      str_detect(CLASS, regex("^wint", ignore_case = TRUE))         ~ NGROW_PET,
      CROP == "TRITICALE" & DOUBLE == "Y"                           ~ NGROW_PET,
      TRUE                                                          ~ GROW_PET
    )
  ) %>%
