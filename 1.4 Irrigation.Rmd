...
HOWEVER, in Canada's methodology, we assumed that irrigation occurs when PET < P during growing season. This means I can determine the irrigation proportion for target crops at county level. If double cropping in the US, PER and P in the nongrowing season were used to determine the presence of irrigation. 
...
# The results show that irrigation presence in growing or non growing seasons is synced. 
# There is no county has irrigation in growing but not in non-growing.

#Calculate the irrigation percentage based on the T/F and area at state level
...
data_area_2022 <- data_2022 %>%
  ...
  filter(STATISTICCAT_DESC == "AREA HARVESTED") %>%
    mutate(
    expected_desc = case_when(
      COMMODITY_DESC == "WHEAT" & CLASS_DESC != "ALL CLASSES" ~ 
        str_c("WHEAT, ", CLASS_DESC, " - ACRES HARVESTED"),
      COMMODITY_DESC == "WHEAT" & CLASS_DESC == "ALL CLASSES" ~
        NA_character_,                                    # <- drop sum of wheat (spring, durum, winter)
      COMMODITY_DESC %in% c("CORN","SORGHUM")  & UTIL_PRACTICE_DESC %in% c("GRAIN", "SILAGE") ~
        str_c(COMMODITY_DESC, ", ", UTIL_PRACTICE_DESC, " - ACRES HARVESTED"),
      COMMODITY_DESC == "PEAS" & CLASS_DESC == "DRY EDIBLE" ~ 
        str_c("PEAS, ", CLASS_DESC, " - ACRES HARVESTED"), #only dry used to biofuel
      TRUE ~ str_c(COMMODITY_DESC, " - ACRES HARVESTED")
    )
  ) %>%
  filter(SHORT_DESC == expected_desc) %>% 
  #select(-expected_desc) %>%  # optional: drop the helper column
  ungroup() %>%
  filter(UNIT_DESC == "ACRES") %>%
  filter(DOMAIN_DESC == "TOTAL") %>%
  mutate(Area_tot = ifelse(grepl("^\\(", VALUE), 0, VALUE)) %>% #keep numeric data only, remove D
  mutate(Area_tot = as.numeric(gsub(",", "", Area_tot))) %>%    #convert to numeric
  group_by(COMMODITY_DESC, SHORT_DESC, STATE_NAME, DOMAIN_DESC) %>%
...
data_Irri_2022 <- data_2022 %>% 
  ...
  mutate(
    expected_desc = case_when(
      # Wheat: keep Winter / Durum / Spring only
      COMMODITY_DESC == "WHEAT" & CLASS_DESC != "ALL CLASSES" ~
        str_c("WHEAT, ", CLASS_DESC, ", IRRIGATED - ACRES HARVESTED"),
      COMMODITY_DESC == "WHEAT" & CLASS_DESC == "ALL CLASSES" ~
      NA_character_,                      # ← mark rows to discard
      # Corn & Sorghum: keep Grain + Silage only
      COMMODITY_DESC %in% c("CORN", "SORGHUM") &
        UTIL_PRACTICE_DESC %in% c("GRAIN", "SILAGE") ~
        str_c(COMMODITY_DESC, ", ", UTIL_PRACTICE_DESC, 
              ", IRRIGATED - ACRES HARVESTED"),
      # Peas: only Dry-edible
      COMMODITY_DESC == "PEAS" & CLASS_DESC == "DRY EDIBLE" ~
        str_c("PEAS, ", CLASS_DESC, ", IRRIGATED - ACRES HARVESTED"),
      # All other crops
      TRUE ~ str_c(COMMODITY_DESC, ", IRRIGATED - ACRES HARVESTED")
    )
  ) %>% 
  filter(SHORT_DESC == expected_desc) %>% 
  filter(
    UNIT_DESC   == "ACRES",
    DOMAIN_DESC == "TOTAL"        # matches irrigated rows
  ) %>% 
  mutate(
    Area_irr = ifelse(grepl("^\\(", VALUE), 0, VALUE),  # drop “(D)” etc.
    Area_irr = as.numeric(gsub(",", "", Area_irr))
  ) %>% 
  group_by(COMMODITY_DESC, SHORT_DESC, STATE_NAME) %>% 
  summarise(Area_irr = sum(Area_irr), .groups = "drop") %>%
...
#Join the data to determine irrigation fraction and merge silage and grain corn
Frac_irri_2022 <- data_area_2022 %>%
  left_join(data_Irri_2022, by = c("COMMODITY_DESC", "STATE_NAME", "SHORT_DESC")) %>%
  mutate(
    SHORT_DESC = str_replace(
      SHORT_DESC,
      "^(CORN|SORGHUM),\\s*(GRAIN|SILAGE)\\s*-\\s*ACRES HARVESTED$",
      "\\1 - ACRES HARVESTED"
    )
  ) %>% 
  group_by(COMMODITY_DESC, SHORT_DESC, STATE_NAME) %>% 
  summarise(
    Area_tot = sum(Area_tot,  na.rm = TRUE),
    Area_irr = sum(Area_irr,  na.rm = TRUE),
    .groups  = "drop"
  ) %>%
  mutate(Frac_irri = ifelse(Area_irr/Area_tot > 1, 1, Area_irr/Area_tot))
...
Frac_irri_recon <- Frac_irri %>%
  ...
  mutate(
    ...
    conflict_class = case_when(
      mean_17_22  == 0 & Irri_percent == 0           ~ "OK",           # mean no irrigation, agree by both methods
      mean_17_22  == 0 & Irri_percent > 0           ~ "Partial",       # It possible that small/partial irrigation occur
    ...
    Frac_irri_final = case_when(
      conflict_class == "Partial"    ~ Irri_percent*0.5, # This is an arbitrary number.
      conflict_class == "A_conflict" ~ mean_17_22, #Use the  average when survey avg showed higher than our estimate
      conflict_class == "B_conflict" ~ (Irri_percent + mean_17_22)/2, #Our method could overestimate irrigation
...
#Create a CropID to join the Direct data later
Frac_irri_recon <- Frac_irri_recon %>% 
  mutate(tmp = str_remove(SHORT_DESC, "\\s*-\\s*ACRES HARVESTED$") %>%
               str_remove(",\\s*DRY EDIBLE") %>%                 
               str_trim()) %>% 
  mutate(
    CropID = case_when(
      str_detect(tmp, "^WHEAT,\\s*SPRING,\\s*\\(EXCL DURUM\\)$") ~ "Wheat(spring)",
      str_detect(tmp, "^WHEAT,\\s*SPRING,\\s*DURUM$")            ~ "Wheat(durum)",
      str_detect(tmp, "^WHEAT,\\s*WINTER$")                      ~ "Wheat(winter)",
      TRUE                                                      ~ str_to_sentence(tmp)
    )
  ) %>% 
  select(-tmp)   # drop the helper column


write.csv(Frac_irri_recon, "Outputs/1.4_Irrigation fraction.csv",row.names=FALSE)
