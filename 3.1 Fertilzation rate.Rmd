...
Production_percentage <- function(data, crop_class) {
  ...
  filtered_data <- data %>%
    ...
    filter((COMMODITY_DESC != "CORN") | (COMMODITY_DESC == "CORN" & UTIL_PRACTICE_DESC %in% c("GRAIN","SILAGE"))) %>%
    filter((COMMODITY_DESC != "SORGHUM") | (COMMODITY_DESC == "SORGHUM" & UTIL_PRACTICE_DESC %in% c("GRAIN","SILAGE"))) %>%
